...
test.describe('SG_Payroll (Playwright using PaymentsPages)', () => {
  ...
  test('TC001_SGPayroll - Create Payroll Alternate with new payee', async ({ page }) => {
    ...
    await pages.PayrollPage.enterTransactionReferences(testData);
     ...
     */
    await pages.PayrollPage.validatePayeeOrRefrenceNoDetailsOfPayroll(testData);
    
    /**
     * This method deletes the existing opened PayeeOrRefrenceNo Details.
     */
    await pages.PayrollPage.deleteOpenPayeeOrReferenceNo(testData);
