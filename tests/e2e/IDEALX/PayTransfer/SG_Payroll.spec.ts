...
test.describe('SG_Payroll (Playwright using PaymentsPages)', () => {
  ...
  test('TC001_SGPayroll - Create Payroll Alternate with new payee', async ({ page }) => {
     ...
     */
    await pages.PayrollPage.validatePayeeOrRefrenceNoDetailsOfPayroll(testData,internalReferenceAutoGenerated=false, reference, amountSGDIsEdit=false, yourAccountDeductedEdited=false);
    
    /**
     * This method deletes the existing opened PayeeOrRefrenceNo Details.
     */
    await pages.PayrollPage.deleteOpenPayeeOrReferenceNo(testData,internalReferenceAutoGenerated=false, reference);
  ...
  test('TC002_SGPayroll - Edit Payroll Alternate via Transfer Center', async ({ page }) => {
    ...
    // Find it again in Transfer Center by reference
    //await pages.AccountTransferPage.safeClick(pages.AccountTransferPage.paymentMenu);
    await webComponents.clickWhenVisibleAndEnabledCustomWait(pages.AccountTransferPage.paymentMenu,75_000);
    await pages.TransferCentersPage.searchAndOpenByReference(reference);
    await pages.PayrollPage.waitForViewPaymentPageReady();

   /**
     * This method validates the existing PayeeOrRefrenceNo Details.
     * Validates values after editing the Amount..
     */
