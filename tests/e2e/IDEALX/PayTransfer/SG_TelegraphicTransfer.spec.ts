...
test('TC001_SG_TelegraphicTransfer - Create A TT Payment With New Payee', { tag: ['@telegraphic-transfer' ] },async ({ page }) => {
...
test('TC002_SG_TelegraphicTransfer - Create A TT Payment With ApprovalNow Pmchallenge', { tag: ['@telegraphic-transfer' ] }, async ({ page }) => {
  // Payments → Transfer Center → Make a Payment
  // paymentMenu => Pay & Transfer (Left option)
  await pages.AccountTransferPage.waitForMenu();
  await pages.AccountTransferPage.safeClick(pages.AccountTransferPage.paymentMenu);
...
test('TC003_SG_TelegraphicTransfer - Create A TT Payment With ApprovalNow Mchallenge', { tag: ['@telegraphic-transfer' ] }, async ({ page }) => {
  // Payments → Transfer Center → Make a Payment
  // paymentMenu => Pay & Transfer (Left option)
  await pages.AccountTransferPage.waitForMenu();
  await pages.AccountTransferPage.safeClick(pages.AccountTransferPage.paymentMenu);
...
test('TC004_SG_TelegraphicTransfer - Create A TT Payment With Save As Template', { tag: ['@telegraphic-transfer' ] }, async ({ page }) => {
  // Payments → Transfer Center → Make a Payment
  // paymentMenu => Pay & Transfer (Left option)
  await pages.AccountTransferPage.waitForMenu();
  await pages.AccountTransferPage.safeClick(pages.AccountTransferPage.paymentMenu);
...
test('TC005_SG_TelegraphicTransfer - Create A TT Payment From Template', { tag: ['@telegraphic-transfer' ] }, async ({ page }) => {
...
test('TC006_SG_TelegraphicTransfer - Create A TT With Save As Draft', { tag: ['@telegraphic-transfer' ] }, async ({ page }) => {
  // Payments → Transfer Center → Make a Payment
  // paymentMenu => Pay & Transfer (Left option)
  await pages.AccountTransferPage.waitForMenu();
  await pages.AccountTransferPage.safeClick(pages.AccountTransferPage.paymentMenu);
...
test('TC007_SG_TelegraphicTransfer - Copy A TT Payment Via Transfer Center', { tag: ['@telegraphic-transfer' ] },async ({ page }) => {
...
test('TC008_SG_TelegraphicTransfer - Edit A TT Payment Via Transfer Center', { tag: ['@telegraphic-transfer' ] }, async ({ page }) => {
  //checkpoint for reference2 which is created in TC001, if it's empty then throw error to skip this test as it has dependency on TC001
  if (!reference2?.trim()) throw new Error('reference2 is empty – TC001 must pass first');
  // Step 1:Payments → Transfer Center → Search for a payment using reference number created in TC001 and open it
  await pages.AccountTransferPage.waitForMenu();
...
test('TC009_SG_TelegraphicTransfer - Reject A TT Payment Via Transfer Center', { tag: ['@telegraphic-transfer' ] },async ({ page }) => {
  //checkpoint for reference2 which is created in TC001, if it's empty then throw error to skip this test as it has dependency on TC001
...
test('TC010_SG_TelegraphicTransfer - Delete A TT Payment Via Transfer Center', { tag: ['@telegraphic-transfer' ] },async ({ page }) => {
  //checkpoint for reference2 which is created in TC001, if it's empty then throw error to skip this test as it has dependency on TC001
  if (!reference2?.trim()) throw new Error('reference2 is empty – TC001/TC008 must pass first');
  // Step 1:Payments → Transfer Center → Search for a payment using reference number created in TC001 and open it
  await pages.AccountTransferPage.waitForMenu();
...
test('TC011_SG_TelegraphicTransfer - Create A TT Payment With Currency As SGD And Payee Bank Supports PARTIOR', { tag: ['@telegraphic-transfer' ] },async ({ page }) => {
...
test('TC012_SG_TelegraphicTransfer - Approve A TT Payment For PARTIOR', { tag: ['@telegraphic-transfer' ] },async ({ page }) => {
...
test('TC013_SG_TelegraphicTransfer - Edit a TT Payment with max amount 999999999.99 CNH', { tag: ['@telegraphic-transfer' ] },async ({ page }) => {
  // Step 1: Payments → Transfer Center → Make a Payment
  // paymentMenu => Pay & Transfer (Left option)
  await pages.AccountTransferPage.waitForMenu();
  await pages.AccountTransferPage.safeClick(pages.AccountTransferPage.paymentMenu);
...
test('TC014_SG_TelegraphicTransfer - Verify A TT Payment Via My Verify', { tag: ['@telegraphic-transfer' ] },async ({ page }) => {
...
test('TC015_SG_TelegraphicTransfer - Approve A TT Payment Via Transfer Center', { tag: ['@telegraphic-transfer' ] },async ({ page }) => {
  //checkpoint for copyreference which is created in TC007, if it's empty then throw error to skip this test as it has dependency on TC007
  if (!copyreference?.trim()) throw new Error('copyreference is empty – TC007 must pass first');
...
test('TC016_SG_TelegraphicTransfer - Release A TT Payment Via My Release', { tag: ['@telegraphic-transfer' ] },async ({ page }) => {
  //checkpoint for copyreference which is created in TC007, if it's empty then throw error to skip this test as it has dependency on TC007
  if (!copyreference?.trim()) throw new Error('copyreference is empty – TC007 must pass first');
...
test('TC017_SG_TelegraphicTransfer - DOL User Create A TT Payment With Showing FX Savings Message Old UI', { tag: ['@telegraphic-transfer' ] }, async ({ page }) => {
  // Step 1: Payments → Transfer Center → Make a Payment
  // paymentMenu => Pay & Transfer (Left option)
  await pages.AccountTransferPage.waitForMenu();
  await pages.AccountTransferPage.safeClick(pages.AccountTransferPage.paymentMenu);
