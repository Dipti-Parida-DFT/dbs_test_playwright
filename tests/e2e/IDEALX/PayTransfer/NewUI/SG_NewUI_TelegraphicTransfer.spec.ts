...
const newUIMode = true;
test.beforeEach(async ({ page }, testInfo) => {
 
  const alt = isAltUser(testInfo.title);
  //await doLogin(page,  new ? testData.TelegraphicTransfer.newUI.loginCompanyId : testData.TelegraphicTransfer.loginCompanyId, new ? testData.TelegraphicTransfer.newUI.loginUserId : testData.TelegraphicTransfer.loginUserId);
  
  const loginPage = new LoginPage(page);
  await loginPage.goto();
  await loginPage.login(newUIMode ? testData.TelegraphicTransfer.newUI.loginCompanyId : testData.TelegraphicTransfer.loginCompanyId, newUIMode ? testData.TelegraphicTransfer.newUI.loginUserId : testData.TelegraphicTransfer.loginUserId, (String(CONSTANTS.PIN)));
...
test('TC001_SG_TelegraphicTransfer - Create A TT Payment With New Payee', async ({ page }) => {
  ...
  //new await pages.TelegraphicTransferPage.safeClick(pages.TelegraphicTransferPage.debitAccountFilter);
  ...
  //new await pages.TelegraphicTransferPage.safeClick(pages.TelegraphicTransferPage.selectOrAddPayeeFilter);
  //new await pages.TelegraphicTransferPage.safeClick(pages.TelegraphicTransferPage.addNewPayeeButton);
...
test('TC001.1_NEWUI_SG_TelegraphicTransfer - Create A TT Payment With New Payee', async ({ page }) => {
  ...
  //await page.waitForTimeout(TIMEOUT.MAX);
  //await webComponents.waitForUXLoading([], page);
  //await pages.TelegraphicTransferPage.waitForTTFormReady();
  // Step 1: Payment from => Select account from "Account" dropdown and enter amount
  //await pages.TelegraphicTransferPage.safeClick(pages.TelegraphicTransferPage.debitAccountFilter);
  await webComponents.clickWhenVisibleAndEnabled(pages.TelegraphicTransferPage.newUIDebitAccountFilter);
  await pages.TelegraphicTransferPage.safeClick(pages.TelegraphicTransferPage.newUIFromAccount);
  await pages.TelegraphicTransferPage.safeFill(pages.TelegraphicTransferPage.newUIFromAccount, fromAccountNP);
  await pages.TelegraphicTransferPage.safeClick(pages.TelegraphicTransferPage.newUINewPayeeBankAccountDropdown);
  //await pages.TelegraphicTransferPage.waitForCurrency();
  await pages.TelegraphicTransferPage.safeClick(pages.TelegraphicTransferPage.amountInput);
  await pages.TelegraphicTransferPage.safeFill(pages.TelegraphicTransferPage.amountInput, testData.TelegraphicTransfer.amountA1);
  //await pages.TelegraphicTransferPage.handleMYRContinueIfPresent();
  // Step 2: Payment from => Click "New payee" tab and enters details
  await pages.TelegraphicTransferPage.safeClick(pages.TelegraphicTransferPage.selectOrAddPayeeFilter);
  await pages.TelegraphicTransferPage.safeClick(pages.TelegraphicTransferPage.addNewPayeeButton);
  await pages.TelegraphicTransferPage.handleMYRContinueIfPresent();
  await pages.TelegraphicTransferPage.newUIAddNewTTPayee({
    name:                     testData.TelegraphicTransfer.name,
    nickName:                 testData.TelegraphicTransfer.nickName,
    add1:                     testData.TelegraphicTransfer.add1,
    add2:                     testData.TelegraphicTransfer.add2,
...
test('TC006.1_NEWUI_SG_TelegraphicTransfer - Create A TT With Save As Draft', async ({ page }) => {
  //Step 1: Navigate Payment & Transfer Menu.
  await webComponents.waitForUXLoading([], page);
  await webComponents.waitElementToBeVisible(pages.AccountTransferPage.paymentMenu);
  await webComponents.clickWhenVisibleAndEnabled(pages.AccountTransferPage.paymentMenu);
  ...
  // Step 3: Click Pay Local option.
  await webComponents.clickWhenVisibleAndEnabled(pages.TelegraphicTransferPage.makePayment);
  await webComponents.clickWhenVisibleAndEnabled(pages.TelegraphicTransferPage.newUIDebitAccountFilter);
  //await pages.TelegraphicTransferPage.safeClick(pages.TelegraphicTransferPage.newUIFromAccount);
  await webComponents.clickWhenVisibleAndEnabled(pages.TelegraphicTransferPage.newUIFromAccount);
  await pages.TelegraphicTransferPage.safeFill(pages.TelegraphicTransferPage.newUIFromAccount, fromAccountNP);
  //await pages.TelegraphicTransferPage.safeClick(pages.TelegraphicTransferPage.newUINewPayeeBankAccountDropdown);
  await webComponents.clickWhenVisibleAndEnabled(pages.TelegraphicTransferPage.newUINewPayeeBankAccountDropdown);
  ...
  // Step 2: Payment from => Click "New payee" tab and enters details
  //await pages.TelegraphicTransferPage.safeClick(pages.TelegraphicTransferPage.selectOrAddPayeeFilter);
  await webComponents.clickWhenVisibleAndEnabled(pages.TelegraphicTransferPage.selectOrAddPayeeFilter);
  
  //await pages.TelegraphicTransferPage.safeClick(pages.TelegraphicTransferPage.addNewPayeeButton);
  await pages.TelegraphicTransferPage.handleMYRContinueIfPresent();
  
  // Step 2: Payment from => Enter details in existing payee form
  await pages.TelegraphicTransferPage.addNewUIExistingTTPayee({
    existingAccountNumber: testData.TelegraphicTransfer.newUI.existingAccountNumber,
    bankChargeType:        testData.TelegraphicTransfer.bankChargeTypeOUR,
    payeeBankMsg:          testData.TelegraphicTransfer.bankMessage,
    email1:                testData.TelegraphicTransfer.emailId0,
    email2:                testData.TelegraphicTransfer.emailId1,
