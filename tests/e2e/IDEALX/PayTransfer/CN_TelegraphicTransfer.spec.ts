/**
  ...
  * 5) TC005_CN_TelegraphicTransfer - Create A CNTT Payment With Save As Template(Type 1)
  * 6) TC006_CN_TelegraphicTransfer - Create A CNTT Payment From Template
  * 7) TC007_CN_TelegraphicTransfer - Create A CNTT Payment With Save As Draft(Trade Related - Not Advance Payment - Declared Customs With No Goods)
  * 8) TC008_CN_TelegraphicTransfer - Copy A CNTT Payment Via Transfer Center
  */
...
test.describe('CN_TelegraphicTransfer (Playwright using PaymentsPages)', () => {
  ...
  */
  ...
  test('TC005_CN_TelegraphicTransfer - Create A CNTT Payment With Save As Template(Type 1)', async ({ page }) => {
    // Step 0: Login and handle to Protection Message
    await pages.TelegraphicTransferPage.waitForProtectionMessage();
    await webComponents.waitForUXLoading([], page);
    // Payments → Transfer Center → Make a Payment
    ...
    await webComponents.clickWhenVisibleAndEnabled(pages.TelegraphicTransferPage.newTTPayeeNextButton);
    await pages.TelegraphicTransferPage.waitForNewTTPreviewPageReady();
    templateNewer = await pages.TelegraphicTransferPage.ttSaveAsTemplate();
    await webComponents.clickWhenVisibleAndEnabled(pages.TelegraphicTransferPage.newTTsubmitButton);
    await pages.TelegraphicTransferPage.waitFornewTTSubmittedPageReady();
    ...
    pages = new PaymentsPages(page);
    //Login and handle to Protection Message
    await pages.TelegraphicTransferPage.waitForProtectionMessage();
    await webComponents.waitForUXLoading([], page);
    // Payments → Transfer Center → Make a Payment
    // paymentMenu => Pay & Transfer (Left option)
  ...
  test('TC006_CN_TelegraphicTransfer - Create A CNTT Payment From Template', async ({ page }) => {
  ...
  test('TC007_CN_TelegraphicTransfer - Create A CNTT Payment With Save As Draft(Trade Related - Not Advance Payment - Declared Customs With No Goods)', async ({ page }) => {
    // Step 0: Login and handle to Protection Message
    await pages.TelegraphicTransferPage.waitForProtectionMessage();
    await webComponents.waitForUXLoading([], page);
    // Payments → Transfer Center → Make a Payment
    ...
    await webComponents.javaScriptsClick(pages.TelegraphicTransferPage.ttSaveAsDraftDismissButton);
    await webComponents.clickWhenVisibleAndEnabled(pages.AccountTransferPage.paymentMenu);
    await pages.TransferCentersPage.waitForTransferCenterReady();
    await pages.TransferCentersPage.searchAndOpenByReference(reference);
    await pages.TelegraphicTransferPage.newTTWaitForViewPaymentPageReady();
  ...
  test('TC008_CN_TelegraphicTransfer - Copy A CNTT Payment Via Transfer Center', async ({ page }) => {
    ...
    await webComponents.waitForUXLoading([], page);
    // Update account details  in copy payment form
    await webComponents.clickWhenVisibleAndEnabled(pages.TelegraphicTransferPage.amountInput);
    ...
    await webComponents.javaScriptsClick(pages.TelegraphicTransferPage.isTaxFreeGoods);
