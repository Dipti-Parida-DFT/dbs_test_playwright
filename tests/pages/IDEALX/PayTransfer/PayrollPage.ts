 ...
 async deletePayeeInRow(textKey: string, confirm = true) {
  ...
  /**
    ...
    async validatePayeeOrRefrenceNoDetailsOfPayroll(testData,internalReferenceAutoGenerated, reference, amountSGDIsEdit,yourAccountDeductedEdited) {
      ...
      await this.webComponents.waitElementToBeVisible(this.hashValueLabel);
      ...
      await this.webComponents.compareUIVsJsonValue(this.paymentType, testData.Payroll.paymentType);
       ...
       //5 Payment Date
       await this.webComponents.waitElementToBeVisible(this.paymentDate);
      ...
      // 8) Your account will be deducted : 
      ...
      //await this.showOptionalViewButton1.waitFor({state: 'visible'});
      //await this.showOptionalViewButton1.evaluate((el: HTMLElement) => {el.click()});
