...
export class NewUITelegraphicTransferPage {
  constructor(private readonly page: Page) {
    ...
    //New UI
    this.newUIDebitAccountFilter = page.locator('//p[text()="Debit account"]');
    this.newUIFromAccount = page.locator('//input[@placeholder="Search account name or account number"]');
    this.newUIDebitAccount = page.locator('//input[@placeholder="Search by name or account number"]');
    this.newUINewPayeeBankAccountDropdown = page.locator('//li[@id="acct-id-0"]/div');

    this.selectOrAddPayeeFilter = page.locator('//p[text()="Select or add payee"]');
    this.addNewPayeeButton = page.locator('//label[text()="Add new payee"]');
    this.newUIEnterNewPayeeBankID = page.locator('//div[@class="payee-bank-search--input"]/input');
    this.newUIFindTTBankIDButton = page.locator('//*[@id="new-payee-bank-id-button"]');
    this.newUIBankIDOption = page.locator('//div[@class="search-result active ng-star-inserted"]');
    this.newUIEnterPayeeBankID = page.locator('//*[@name="new-payee-bank-id"]');
    this.newUIPayeeLocation = page.locator('//input[@id="new-payee-address-country"]');
    this.newUIPayeeTown = page.locator('//input[@name="new-payee-town"]');
    this.newUITTPayeeAddress1 = page.locator('//*[@name="new-payee-add1"]');
    this.newUITTPayeeAddress2 = page.locator('//*[@name="new-payee-add2"]');
    this.newUIPostalCode = page.locator('//input[@name="new-payee-postalCode"]');
    this.newUIExistingPayeeBankAccount = page.locator('//input[@id="existing-payee-filter"]');
    this.newUIExistingPayeeBankAccountSelect = page.locator('//div[@class="payee-name"]');
    this.newUIErrorMessage = page.locator('(//div[contains(@class,"information-banner__flex-vertical")]/div/p)[1]');
    this.newUIFindIntermediaryBankIDButton = page.locator('//div[@class="payee-bank-search--input"]/input');
    ...
    //New UI
    readonly newUIDebitAccountFilter: Locator;
    readonly newUIFromAccount: Locator;
    readonly newUIDebitAccount: Locator;
    readonly newUINewPayeeBankAccountDropdown: Locator;
    readonly debitAccountFilter: Locator;
    readonly selectOrAddPayeeFilter: Locator;
    readonly addNewPayeeButton: Locator;
    readonly newUIEnterNewPayeeBankID: Locator;
    readonly newUIFindTTBankIDButton: Locator;
    readonly newUIBankIDOption: Locator;
    readonly newUIEnterPayeeBankID: Locator;
    readonly newUIPayeeLocation: Locator;
    readonly newUIPayeeTown: Locator;
    readonly newUITTPayeeAddress1: Locator;
    readonly newUITTPayeeAddress2: Locator;
    readonly newUIPostalCode: Locator;
    readonly newUIExistingPayeeBankAccount: Locator;
    readonly newUIExistingPayeeBankAccountSelect: Locator;
    readonly newUIErrorMessage: Locator;
    readonly newUIFindIntermediaryBankIDButton: Locator;
  ...
  async addNewTTPayee(input: NewTTPayeeInput): Promise<NewTTPayeeResult> {
   ...
   async newUIAddNewTTPayee(input: NewTTPayeeInput): Promise<NewTTPayeeResult> {
    const { name, nickName, city, add1, add2, bankId, routingCode, accountNumber, intermediaryBankLocation, intermediaryBankId, bankChargeType, purposeCode, payeeBankMsg, email1, email2, email3, email4, email5, payeeMsg, additionalNote, remitterIdentity } = input;
      //await this.safeClick(this.newPayeeTab)
      //await this.handleCognitiveContinueIfPresent();
      //await this.waitForNewPayeeFormReady();
      await this.newUIEnterNewPayeeBankID.click();
      await this.newUIEnterPayeeBankID.fill(bankId);
      await this.page.keyboard.press('Enter');
      await this.newUIEnterPayeeBankID.blur();
      //await this.safeClick(this.newUIFindTTBankIDButton);
      await this.safeClick(this.newUIBankIDOption);
      ...
      await this.safeClick(this.newUIPayeeLocation);
      await this.safeClick(this.MYRlocation);
      await this.newUIPayeeLocation.blur();

      await this.safeClick(this.newUIPayeeTown);
      await this.safeFill(this.newUIPayeeTown, city);
      await this.page.keyboard.press('Tab');
      await this.newUIPayeeTown.blur();

      await this.safeClick(this.newUITTPayeeAddress1);
      await this.safeFill(this.newUITTPayeeAddress1, add1);
      await this.page.keyboard.press('Tab');
      await this.newUITTPayeeAddress1.blur();

      await this.safeClick(this.newUITTPayeeAddress2);
      await this.safeFill(this.newUITTPayeeAddress2, add2);
      await this.page.keyboard.press('Tab');
      await this.newUITTPayeeAddress2.blur();
  ...
  async addNewUIExistingTTPayee(input: ExistingTTPayeeInput): Promise<ExistingTTPayeeResult> {
    const {existingAccountNumber, bankChargeType, payeeBankMsg, email1, email2, email3, email4, email5, payeeMsg, additionalNote, remitterIdentity } = input;
      //await this.safeClick(this.existingPayeeTab)

      await this.safeClick(this.newUIExistingPayeeBankAccount);
      await this.safeFill(this.newUIExistingPayeeBankAccount, existingAccountNumber);
      await this.newUIExistingPayeeBankAccount.blur();
      await this.safeClick(this.newUIExistingPayeeBankAccountSelect);
